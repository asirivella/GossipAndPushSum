...
case class BuildNeighbours(neighbourActors: ArrayBuffer[ActorRef]) extends Information
case class ImplementGossip() extends Information
case class ImplementPushSum(s: Double, w: Double) extends Information
case class NodeTerminated() extends Information
case class MessagePropSuccess() extends Information
case class NeighbourTerminate() extends Information
case class SpreadPushSum() extends Information
case class PrintNeighbours() extends Information
case class SpreadGossip() extends Information
case class StartGossip() extends Information
case class ShutDownNodes() extends Information
case class BuildForPushSum(s: Double, w: Double) extends Information
case class Discard(neighbour: ActorRef) extends Information


object GossipImplBonus extends App{

	if(args.isEmpty || args(0).isEmpty() || args(1).isEmpty() || args(2).isEmpty() || args(3).isEmpty()){
		println("Enter arguments for NumberOfNodes Topology Algorithm");
	}else{
		InitializeAndInitiate(args(0).toInt, args(1), args(2), args(3).toInt)
	}

	def InitializeAndInitiate(numNodes: Int, topology: String, algorithm: String, intervalOfFailure: Long) 
	{
		val system = ActorSystem("InformationSystem")
				val master = system.actorOf(Props(new Master(numNodes, topology, algorithm, intervalOfFailure)), name = "Master")
				master ! StartGossip
	}
}

class Master (totalNumNodes: Int, topology: String, algorithm: String, intervalOfFailure: Long) extends Actor{
	import context._
  ...
  var listOfNodesShutDown : ArrayBuffer[Int] = new ArrayBuffer[Int]()
	...
	for (i <- 0 until totalNumNodes) 
      ...
      context.system.scheduler.scheduleOnce(Duration.create(intervalOfFailure, TimeUnit.MILLISECONDS), self, ShutDownNodes)
		}
		...
		case ShutDownNodes =>
		{
			var nodeIndex = Random.nextInt(aliveActors.length)
					while(listOfNodesShutDown.contains(nodeIndex))
						nodeIndex = Random.nextInt(aliveActors.length)
						aliveActors -= AllActors(nodeIndex)
						context.stop(AllActors(nodeIndex))
						listOfNodesShutDown += nodeIndex
						println("Node with index " + nodeIndex + " has been shut down permanently")
						context.system.scheduler.scheduleOnce(Duration.create(intervalOfFailure, TimeUnit.MILLISECONDS), self, ShutDownNodes)
		}
